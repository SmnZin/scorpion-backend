...
citaCtrl.getHorariosDisponibles = async (req, res) => {
  ...
  try {
    ...
    //setemos dos fechas para que la busqueda sea entre las 00:00 y las 23:59
    const fechaInicio = new Date(fechaDeseada.setHours(0, 0, 0, 0));
    const fechaFin = new Date(fechaDeseada.setHours(23, 59, 59, 999));

    // Hace una consulta a la base de datos para obtener todas las citas que se encuentren en el rango de fechas 
...
citaCtrl.obtenerCitas = async (req, res) => {
  ...
  res
    .status(201)
    .json({ mensaje: "Citas obtenidas con éxito", citas: citas });
